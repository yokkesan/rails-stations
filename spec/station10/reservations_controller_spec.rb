  ...
  describe 'Station10 POST /reservation/' do
    ...
    it 'DBのunique制約にかかったときにリダイレクトテスト' do
      ...
      expect(response).to redirect_to(
        "http://test.host/movies/#{movie.id}/reservation?schedule_id=#{schedule.id}&date=#{date}"
      ).or redirect_to(
        "http://test.host/movies/#{movie.id}/reservation?date=#{date}&schedule_id=#{schedule.id}"
      )
    end
  end
end
